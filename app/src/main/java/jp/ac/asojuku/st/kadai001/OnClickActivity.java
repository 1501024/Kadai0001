...
implements View.OnClickListener{
    ...
    protected void onResume(){
        ...
        // 選択された文字列をキーワードを元に取得
        String checkWord = intent.getStringExtra("usersOption");
        // 正解を文字列定義リソースから取得
        String correctWord = getResources().getString(R.string.usagi);

        // テキストビューの表示はデフォルトは不正解にしてあるので、正解時に表示を変更
        if(correctWord.equals(checkWord)) {
            TextView tvAnsMsg =(TextView)findViewById(R.id.tvAnsMsg);
            tvAnsMsg.setText(getResources().getString(R.string.seikaiMsg));
        }
